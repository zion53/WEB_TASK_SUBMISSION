const productsData=[
    ...
    function productTemp(products) {
     
        check=products.title;

        if((check.toUpperCase().indexOf(filter.value.toUpperCase()) > -1))
   { 
   return `
       <div class="product" >
       <img class="thumbnail" src="${products.thumbnail}">
       
       <h2 class="title">${products.title}</h2>
      
       <ul class="pdetail">
       <li  class="brand">brand:${products.brand}</li> 
       <li  class="bcat">"${products.category}"</li>
       <li  class="bprice">price:${products.price}</li> 
       <li>discount(%):${products.discountPercentage}</li>
       <li  class="bstock">stock:${products.stock}</li>
       <li class="brating">rating:${products.rating}</li>
       </ul>
   ...
   function searchFun() {
      ...
      ${productsData.map(productTemp).join('')}
      <footer> this is the final list keep visiting for more updates</footer>`;
    }
    document.getElementById("productslist").innerHTML = `
   ${productsData.map(productTemp).join('')}
   <footer> this is the final list keep visiting for more updates</footer>`;